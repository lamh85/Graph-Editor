...
const LowerOrderApp = props => {
  ...
  return (
    <>
      <button onClick={startAutoSave}>Start interval</button>
      <button onClick={endAutoSave}>Stop interval</button>
      <div>Is interval set: {JSON.stringify(didStartAutoSave)}</div>
      <PositionWrapper>
        <DrawingsContainer
          width={SVG_WIDTH}
          height={SVG_HEIGHT}
